...
volatile float        exInt, eyInt, ezInt;                   /* error integral     误差积分*/
static volatile float gx, gy, gz, ax, ay, az, mx, my, mz;  
volatile uint32_t     last_update, now_update;               /* Sampling cycle count, ubit ms    采样周期计数，ubit ms*/
static uint8_t        tx, rx;
static uint8_t        tx_buff[14] = { 0xff };
uint8_t               mpu_buff[14];                          /* buffer to save imu raw data    缓冲区保存imu原始数据*/
uint8_t               ist_buff[6];                           /* buffer to save IST8310 raw data    缓冲以保存IST8310原始数据*/
mpu_data_t            mpu_data;
imu_t                 imu={0};
imu_t                 last_imu={0};
/**
  * @brief  fast inverse square-root, to calculate 1/Sqrt(x)      快速逆平方根，计算1/√(x)
  * @param  x: the number need to be calculated   x:这个数字需要计算
  * @retval 1/Sqrt(x)
  * @usage  call in imu_ahrs_update() function     调用imu_ahrs_update()函数
  */
...
/**
  * @brief  write a byte of data to specified register    将一个字节的数据写入指定的寄存器
  * @param  reg:  the address of register to be written    注册表:填写注册地址     
  *         data: data to be written                       数据:需要写入的数据
  * @retval 
...
/**
  * @brief  read a byte of data from specified register    从指定寄存器中读取数据的字节
  * @param  reg: the address of register to be read        要读取的寄存器地址
  * @retval 
...
/**
  * @brief  read bytes of data from specified register    从指定寄存器读取数据字节
  * @param  reg: address from where data is to be written    reg:写入数据的地址
  * @retval 
...
/**
	* @brief  write IST8310 register through MPU6500's I2C master   通过MPU6500的I2C主机编写IST8310寄存器
  * @param  addr: the address to be written of IST8310's register 地址:IST8310的注册地址
  *         data: data to be written                              数据:需要写入的数据
	* @retval   
...
/**
	* @brief  write IST8310 register through MPU6500's I2C Master  通过MPU6500的I2C主机编写IST8310寄存器
	* @param  addr: the address to be read of IST8310's register   地址:IST8310的注册地址
	* @retval 
  * @usage  call in ist8310_init() function
	*/
...
/**
	* @brief    initialize the MPU6500 I2C Slave 0 for I2C reading.    初始化MPU6500的I2C从属0以进行I2C读取。
* @param    device_address: slave device address, Address[6:0]       device_address:从设备地址，地址[6:0]
	* @retval   void
	* @note     
	*/
...
/**
	* @brief  Initializes the IST8310 device   初始化IST8310设备
	* @param  
	* @retval 
...
/**
	* @brief  get the data of IST8310                         获取IST8310的数据
  * @param  buff: the buffer to save the data of IST8310    buff:用来保存IST8310数据的缓冲区
	* @retval 
...
/**
	* @brief  get the data of imu   得到imu的数据
  * @param  
	* @retval 
...
/**
	* @brief  set imu 6500 gyroscope measure range   设置imu 6500陀螺仪测量范围
  * @param  fsr: range(0,±250dps;1,±500dps;2,±1000dps;3,±2000dps)    fsr:范围(0±250 dps; 1±500 dps; 2,±1000 dps; 3、±2000 dps)
	* @retval 
  * @usage  call in mpu_device_init() function
	*/
...
/**
	* @brief  set imu 6050/6500 accelerate measure range   设置imu 6050/6500加速测量范围
  * @param  fsr: range(0,±2g;1,±4g;2,±8g;3,±16g)         fsr:范围(0±2 g, 1±4 g; 2,±8 g; 3、±16 g)
	* @retval 
  * @usage  call in mpu_device_init() function
	*/
...
/**
	* @brief  initialize imu mpu6500 and magnet meter ist3810   初始化imu mpu6500和电磁式仪表ist3810
  * @param  
	* @retval 
  * @usage  call in main() function
	*/
uint8_t mpu_device_init(void)
{
	...
	uint8_t i                        = 0;
	uint8_t MPU6500_Init_Data[10][2] = {{ MPU6500_PWR_MGMT_1, 0x80 },     /* Reset Device   复位装置*/ 
																			{ MPU6500_PWR_MGMT_1, 0x03 },     /* Clock Source - Gyro-Z   时钟源-陀螺- z*/ 
																			{ MPU6500_PWR_MGMT_2, 0x00 },     /* Enable Acc & Gyro   启用Acc和陀螺仪*/ 
																			{ MPU6500_CONFIG, 0x04 },         /* LPF 41Hz   LPF 41赫兹 */ 
																			{ MPU6500_GYRO_CONFIG, 0x18 },    /* +-2000dps */ 
																			{ MPU6500_ACCEL_CONFIG, 0x10 },   /* +-8G */ 
																			{ MPU6500_ACCEL_CONFIG_2, 0x02 }, /* enable LowPassFilter  Set Acc LPF   启用低通滤波器集Acc LPF*/ 
																			{ MPU6500_USER_CTRL, 0x20 },};    /* Enable AUX   启用辅助*/ 
	for (i = 0; i < 10; i++)
	{
...
/**
	* @brief  get the offset data of MPU6500    得到MPU6500的偏移数据
  * @param  
	* @retval 
  * @usage  call in main() function
	*/
...
void imu_attitude_update(void)
{
	...
	imu.yaw = RingConvertToRF(360,imu.yaw);
//	if (imu.yaw <0)
//		imu.yaw +=360;
	/* pitch  -pi/2----pi/2 */
a = -asin(-2*q1*q3 + 2*q0*q2)* 57.3+5.5;   
	/* roll   -pi----pi  */	
	imu.rol =  atan2(2*q2*q3 + 2*q0*q1, -2*q1*q1 - 2*q2*q2 + 1)* 57.3;
	
  if (HAL_GetTick ()==3500)
		pit=imu.pit;
	if (HAL_GetTick ()==13500)
	{
		
		delta=(imu.pit-pit)/10000;
		imu.pit=0;
	}
	imu.pit=imu.pit+(-(imu.wy*57.3f+last_imu .wy*57.3f )*0.001f/2-delta );

}

void imu_heat()
{
	pid_calc(&imu_pwm_pid,imu.temp ,IMU_TEMPRETURE);
	__HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_2, imu_pwm_pid.pos_out );
}
float imuerryaw = 0;
void imu_task()
{
	...
	imu_ahrs_update();
	imu_attitude_update(); 
	if (fabs(last_imu.yaw - imu.yaw)<0.03)
	{
		imuerryaw += last_imu.yaw - imu.yaw;
	}
	imu.yaw -= imuerryaw;
